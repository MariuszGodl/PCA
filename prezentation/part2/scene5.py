...
from manim_slides import Slide  # Added manim_slides import

import numpy as np
import matplotlib.pyplot as plt


class Final2_5(Slide, VoiceoverScene):
    def construct(self):


        # ---------------------------------------------------------
        # 1. MNIST visualisation
        # ---------------------------------------------------------
        axes = Axes(
            x_range=[0, 1, 0.5],
            y_range=[0, 1, 0.5],
        ...
        # ---------------------------------------------------------
        # 2. Time comparison of the different methods
        # ---------------------------------------------------------
        pca_times = [
            0.03408479690551758, 0.04994630813598633, 0.09099149703979492,
            0.13801074028015137, 0.15573668479919434, 0.14432907104492188,
            0.1967780590057373, 0.3617284297943115, 0.3464477062225342,
            0.546889066696167,
        ]

        tsne_times = [
            5.03499174118042, 12.883524656295776, 20.816063404083252,
            29.46470046043396, 39.37271237373352, 48.95806956291199,
            59.08102488517761, 68.41888761520386, 77.59302854537964,
            87.58669137954712,
        ]

        umap_times = [
            1.6609816551208496, 3.59382963180542, 6.724499464035034,
            9.919588088989258, 5.05280613899231, 6.045594215393066,
            6.93833327293396, 7.948130130767822, 8.86552357673645,
            10.15117073059082,
        ]
        ...
        # ---------------------------------------------------------
        # 3. Evolution of the number of neighbors
        # ---------------------------------------------------------
        ...
        mammoth_img = ImageMobject("img/mammoth_3d.png")
        mammoth_img.scale_to_fit_height(5) 
        mammoth_img.next_to(title_3d_ul, direction=DOWN, buff=0.5)

        data_3 = np.load("data/mammoth_umap_3.npy", allow_pickle=True)
        data_3 = (data_3 - np.min(data_3, axis=0)) / (np.max(data_3, axis=0) - np.min(data_3, axis=0))
        
        data_5 = np.load("data/mammoth_umap_5.npy", allow_pickle=True)
        data_5 = (data_5 - np.min(data_5, axis=0)) / (np.max(data_5, axis=0) - np.min(data_5, axis=0))
        
        data_10 = np.load("data/mammoth_umap_10.npy", allow_pickle=True)
        data_10 = (data_10 - np.min(data_10, axis=0)) / (np.max(data_10, axis=0) - np.min(data_10, axis=0))
        
        data_15 = np.load("data/mammoth_umap_15.npy", allow_pickle=True)
        data_15 = (data_15 - np.min(data_15, axis=0)) / (np.max(data_15, axis=0) - np.min(data_15, axis=0))
        
        data_20 = np.load("data/mammoth_umap_20.npy", allow_pickle=True)
        data_20 = (data_20 - np.min(data_20, axis=0)) / (np.max(data_20, axis=0) - np.min(data_20, axis=0))
        
        data_50 = np.load("data/mammoth_umap_50.npy", allow_pickle=True)
        data_50 = (data_50 - np.min(data_50, axis=0)) / (np.max(data_50, axis=0) - np.min(data_50, axis=0))
        
        data_100 = np.load("data/mammoth_umap_100.npy", allow_pickle=True)
        data_100 = (data_100 - np.min(data_100, axis=0)) / (np.max(data_100, axis=0) - np.min(data_100, axis=0))
        ...
        def get_dots(data_array):
            d = VGroup()
            for x, y in data_array:
                d.add(Dot(axes.c2p(x, y), color=BLUE, radius=0.025, fill_opacity=0.6))
            return d

        dots = get_dots(data_3)
        dots_5 = get_dots(data_5)
        dots_10 = get_dots(data_10)
        dots_15 = get_dots(data_15)
        dots_20 = get_dots(data_20)
        dots_50 = get_dots(data_50)
        dots_100 = get_dots(data_100)

        self.play(FadeIn(axes, title_proj, title_proj_ul, title_3d, title_3d_ul, mammoth_img))
        self.play(FadeIn(dots))
        ...
        self.next_slide()
        self.play(FadeOut(axes, title_proj, title_proj_ul, title_3d, title_3d_ul, dots, mammoth_img))
        self.clear() 
