...
\begin{document}
	...
	\begin{frame}{Nasz Cel}
		...
		Naszym zadaniem będzie znalezienie takiego wektora $w$, który zmaksymalizuje wariancję, gdy rzutujemy na niego punkty $X_i$. Przy tym założymy, że dane są znormalizowane (wszystkie cechy mieszczą się w przedziale $[0,1]$) i mają średnią $\overline{X_i} = 0$.\vspace{20pt}
	...
	\begin{frame}{Obliczanie Wariancji 1}
		...
		\begin{equation*}
			\forall_{i\in\{1,\ldots, d\}} i\neq j \implies v_i\perp v_j, \quad \quad \|v_i\| = 1
		\end{equation*}
		\begin{equation*}
	...
	\begin{frame}{Obliczanie Wariancji 3}
		Podstawiamy nowy wzór na $\mathbf{w}$, pamiętając, że wektory własne spełniają równanie $S{}v = \lambda{}v$.
		\begin{equation*}
	...
	\begin{frame}
		\Huge Alternatywne wyprowadzenie --- mnożniki Lagrange'a
	\end{frame}
	
	\begin{frame}{Optymalizacja}
		Naszym celem jest zmaksymalizowanie wariancji, zapisanej jako $\sigma^2 = w^TSw$, gdzie $\|w\|^2 = w^Tw = 1$.
		\begin{equation*}
			\pdv{(w^TSw)}{w} = \lambda \pdv{(w^Tw)}{w}
		\end{equation*}
		\begin{equation*}
			2Sw = 2\lambda w \implies Sw=\lambda w
		\end{equation*}
		
		Z tego wnioskujemy, że wektor maksymalizujący wariancję, jest wektorem własnym macierzy $S$. Pondato
		\begin{equation*}
